...
//			$ta = 2 / pi();
	...
	public static function ASCIICODE($characters) {
		...
		if ((function_exists('mb_strlen')) && (function_exists('mb_substr')) && (mb_strlen($characters, 'UTF-8') > 0)) {
			return self::_uniord(mb_substr($characters, 0, 1, 'UTF-8'));
		} else {
		...
		if (is_array($cellAddress)) {
			foreach($cellAddress as $columnKey => $rowValue) {
				foreach($rowValue as $rowKey => $cellValue) {
					return preg_replace('/[^0-9]/i','',$rowKey);
				}
			}
		} else {
			list($cellAddress) = explode(':',$cellAddress);
			return preg_replace('/[^0-9]/','',$cellAddress);
		}
	}	//	function ROW()
