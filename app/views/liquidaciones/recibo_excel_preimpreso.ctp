 ...
 */
    ...
    foreach ($this->data as $receipt) {
        ...
        for ($i = 0; $i <= 1; $i++) {
			...
			$documento->setCellValue(($spaces + 16) . ',' . $row . ':'. ($spaces + 26) .',' . $row, $receipt['Liquidacion']['empleador_direccion']);
			$documento->moveCurrentRow();
			$documento->activeSheet->getRowDimension(($documento->getCurrentRow() + 1))->setRowHeight(10);
			$row = ($documento->getCurrentRow() + 1);
			$documento->setCellValue(($spaces + 16) . ',' . $row . ':'. ($spaces + 26) .',' . $row, 'C.U.I.T.: ' . $receipt['Liquidacion']['empleador_cuit']);
			...
			$tmp = explode('-', $receipt['Liquidacion']['trabajador_cuit']);
			$dni = $tmp[1];
			$documento->setCellValue(($spaces + 18) . ',' . $row . ':' . ($spaces + 21) .',' . $row, $dni);
