...
$condiciones['Condicion.Pago-relacion_id'] = array(	"lov"=>array("controller"	=>	"relaciones",
																		"camposRetorno"	=>array("Empleador.nombre",
																								"Trabajador.apellido")));
//$condiciones['Condicion.Liquidacion-empleador_nombre'] = array("label"=>"Empleador");
//$condiciones['Condicion.Liquidacion-trabajador_nombre'] = array("label"=>"Trabajador (nombre)");
//$condiciones['Condicion.Liquidacion-trabajador_apellido'] = array("label"=>"Trabajador (apellido)");
...
foreach ($registros as $k=>$v) {
	$fila = null;
	$fila[] = array("tipo"=>"desglose", "id"=>$v['Liquidacion']['id'], "update"=>"desglose1", "imagen"=>array("nombre"=>"liquidaciones.gif", "alt"=>"liquidacion"), "url"=>'../liquidaciones/recibo_html');
	...
	if($v['Pago']['estado'] == "Pendiente" && $v['Pago']['moneda'] == "Pesos") {
		$fila[] = array("tipo"=>"accion", "valor"=>$formulario->link($formulario->image("cheques.gif"), "../pagos_formas/add/PagosForma.forma:Cheque/PagosForma.pago_id:" . $v['Pago']['id'], array("title"=>"Pago con Cheque")));
	}
	elseif($v['Pago']['estado'] == "Imputado") {
		$fila[] = array("tipo"=>"accion", "valor"=>$formulario->link($formulario->image("revertir_pago.gif"), "revertir_pago/" . $v['Pago']['id'], array("title"=>"Revertir Pago")));
	}
	...
	if($v['Pago']['estado'] == "Imputado" || $v['Pago']['estado'] == "Cancelado") {
		$cuerpo[] = array("contenido"=>$fila, "opciones"=>array("seleccionMultiple"=>false));
	}
	elseif(empty($v['Relacion']['Trabajador']['cbu'])) {
		$cuerpo[] = array("contenido"=>$fila, "opciones"=>array("title"=>"No se podra generar el archivo de soporte por no tener cuenta bancaria", "class"=>"fila_resaltada"));
	}
	else {
		$cuerpo[] = $fila;
	}
...
$acciones[] = $formulario->link("Soporte Mag.", null, array("id"=>"generar_soporte_magnetico", "class"=>"link_boton", "title"=>"Generar Soporte Magnetico", "alt"=>"Generar Soporte Magnetico"));
$acciones[] = $formulario->link("Efectivo", null, array("id"=>"pago_efectivo", "class"=>"link_boton", "title"=>"Realiza un pago masivo en Efectivo", "alt"=>"Realiza un pago masivo en Efectivo"));
$acciones[] = $formulario->link("Beneficios", null, array("id"=>"pago_beneficios", "class"=>"link_boton", "title"=>"Realiza un pago masivo con Beneficios", "alt"=>"Realiza un pago masivo con Beneficios"));
$acciones[] = $formulario->link("Deposito", null, array("id"=>"pago_deposito", "class"=>"link_boton", "title"=>"Realiza un pago masivo con Deposito en la Cuenta del Trabajador", "alt"=>"Realiza un pago masivo con Deposito en la Cuenta del Trabajador"));
$accionesExtra['opciones'] = array("acciones"=>$acciones);
$botonesExtra = $formulario->button("Det. Cambio", array("id"=>"detalle_cambio", "title"=>"Imprime el Detalle de Cambio"));
echo $this->renderElement("index/index", array("botonesExtra"=>array("opciones"=>array("botones"=>array("limpiar", "buscar", $botonesExtra))), "accionesExtra"=>$accionesExtra, "opcionesTabla"=>array("tabla"=>array("eliminar"=>false, "modificar"=>false)), "condiciones"=>$fieldset, "cuerpo"=>$cuerpo));
