...
if (!empty($data)) {
    ...
    $documento->create(array('password' => true, 'header' => $header));
    ...
    foreach ($data as $record) {
        ...
        foreach ($record['VacacionesDetalle'] as $detail) {
            ...
            $documento->setCellValue('O', 'Córdoba, ' . $formato->format(Dates::dateAdd($detail['desde'], -45), array('type' => 'date', 'format' => 'd/m/Y')), 'right');
            ...
            $documento->setCellValue('K', 'Documento');
