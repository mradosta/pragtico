...
foreach ($this->data['Concepto'] as $k=>$v) {
	...
	$fila[] = array("tipo"=>"accion", "valor"=>$appForm->link($appForm->image('quitar.gif', array('alt' => "Quitara este concepto de todos los Trabajadores", "title"=>"Quitara este concepto de todos los Trabajadores")), array("action"=>"manipular_concepto/quitar", "convenio_id"=>$this->data['Convenio']['id'], "concepto_id"=>$v['id']), array(), "Quitara este concepto de todos los trabajadores de todos los empleadores que tengan el convenio colectivo '" . $this->data['Convenio']['nombre'] . "'. Desea continuar?"));
